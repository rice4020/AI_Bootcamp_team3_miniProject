...
export async function GET(request) {
  ...
  const radiusParam = searchParams.get('radius') || '3'; // 기본 3km 반경

  const lat = parseFloat(latParam || '37.5665');
  ...
  if (IS_MOCK_MODE) {
    ...
    const nearby = FALLBACK_EVENTS.filter(evt => getDistanceKm(lat, lng, evt.latitude, evt.longitude) <= radius);
  ...
  try {
    ...
    // 3. 불러온 DB 행사 목록에 대하여 3km 반경 연산 필터링 실행
    const formatted = dbEvents.map(row => ({
      ...
      const distance = getDistanceKm(lat, lng, evt.latitude, evt.longitude);
    ...
    console.warn("⚠️ [API/events] DB 오류로 인해 예비 10대 행사 데이터를 계산해 출력합니다:", dbError.message);
    const nearby = FALLBACK_EVENTS.filter(evt => getDistanceKm(lat, lng, evt.latitude, evt.longitude) <= radius);
    return NextResponse.json({ success: true, isMock: true, data: nearby });
  }
}
